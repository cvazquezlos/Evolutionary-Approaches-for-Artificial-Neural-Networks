...
data <- NULL
epochs <- 500
fitness_calculations <- data.frame(individual = character(), gen_check = integer(), mae = double(), loss = double(), 
...
evaluation <- function(word) {
  ...
  history <- model %>% fit(rbind(X_train, X_validation), rbind(y_train, y_validation), validation_split = 0.235294, epochs = epochs, verbose = 0, callbacks = list(
    callback_early_stopping(monitor = 'val_loss', min_delta = 0, patience = 10, verbose = 1, mode = 'auto')
  ))
  model_name <- paste0('gp_models/', j, '/', gen_no, '/', gsub("\"", "", gsub("/", "_", w)), '.h5')
  save_model_hdf5(model, model_name)
  score <- model %>% evaluate(X_validation, y_validation)
  print(score)
  fitness_calculations[nrow(fitness_calculations) + 1,] <<- c(w, gen_no, score['mean_absolute_error'][[1]], score['loss'][[1]], 
                                                             model_name, gsub(" ", "", toString(toJSON(as.data.frame(history)))))
  return(round(double(score['loss'][[1]]), 5))
}
...
results <- data.frame(sol_train_mse = double(),
                      sol_validation_mse = double(),
                      sol_test_mse = double(),
...
for (i in c(1:1)) {
  ...
  optimal_word <- GrammaticalEvolution(grammarDef, evaluation, popSize = 20, mutationChance = 0.05, 
                                       monitorFunc = monitor, iterations = 1, terminationCost = 0.03000)
  end_time <- Sys.time()
  optimal_word_l <- extract_neurons(optimal_word, 0)
  optimal_word_w <- extract_neurons(optimal_word, 1)
  ordered_fitness_calculations <- fitness_calculations[order(fitness_calculations$mae),]
  ...
  sol_train_mse <- score['mean_absolute_error'][[1]]
  score <- model %>% evaluate(X_validation, y_validation)
  sol_validation_mse <- score['mean_absolute_error'][[1]]
  score <- model %>% evaluate(X_test, y_test)
  sol_test_mse <- score['mean_absolute_error'][[1]]
  ...
  j <- j + 1
  results[nrow(results) + 1,] <- c(sol_train_mse, sol_validation_mse, sol_test_mse, 
