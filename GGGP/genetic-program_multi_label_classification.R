...
evaluation <- function(word) {
  ...
  history <- model %>% fit(rbind(X_train, X_validation), rbind(y_train, y_validation), validation_split = 0.222222, epochs = epochs, verbose = 0, callbacks = list(
    callback_early_stopping(monitor = 'val_loss', min_delta = 0, patience = 20, verbose = 1, mode = 'auto')
  ))
  model_name <- paste0('gp_models/', j, '/', gen_no, '/', gsub("\"", "", gsub("/", "_", w)), '.h5')
  save_model_hdf5(model, model_name)
  score <- model %>% evaluate(X_validation, y_validation)
  print(paste0(w, " Accuracy: ", score['acc'][[1]], " Loss: ", round(score['loss'][[1]], 2)))
...
for (i in c(1:1)) {
  ...
  dir.create(paste0("gp_models/", j, "/", gen_no), showWarnings = FALSE)
  start_time <- Sys.time()
  optimal_word <- GrammaticalEvolution(grammarDef, evaluation, popSize = 18, elitism = 2, mutationChance = 0.05, 
                                       monitorFunc = monitor, iterations = 2)
  ...
  or <<- ordered_fitness_calculations[ordered_fitness_calculations$individual == optimal_word_w,]
  ...
  print(optimal_individual$saved_model)
  ...
  sol_loss <- ordered_fitness_calculations$loss
  ...
  j <- j + 1
  results[nrow(results) + 1,] <- c(sol_train_accuracy, sol_validation_accuracy, sol_test_accuracy, sol_loss,
