...
% DEEP LEARNING
  ...
  El algoritmo para la creación de los individuos que se evolucionan a lo largo del programa que se ha seleccionado es el aleatorio, es decir, que siguiendo la gramática mostrada en la Eq. \ref{eq:3}, los individuos se generan a partir de reglas de producción aleatorias y sin una profundidad máxima determinada. Su funcionamiento puede verse en el algoritmo 1.\par
  ...
  \begin{itemize}
    ...
    \item Operador de reemplazo. Tras aplicar los tres operadores anteriores, el operador de reemplazo elegido es por reducción elitista, en el que la población y los descendientes se juntan y se seleccionan los $\lambda$ mejores.
  \end{itemize} \par
  En los siguientes algoritmos se pueden visualizar las implementaciones seguidas para cada uno de los operadores. Estas implementaciones son ideales, y en la práctica se han utilizado funciones más eficientes que el propio lenguaje $R$ provee.
 
 
  
  \begin{algorithm}
    \caption{Algoritmo del operador de mutación}\label{mutaop}
    \begin{algorithmic}[1]
      \Function{crossover}{$f$} \Comment{\textit{f} = Progenitores}
        \State $no\_terminales1 \gets \thinspace $extraer(\textit{f[0]}) \Comment{Extraer símbolos no terminales}
        \State $no\_terminales2 \gets \thinspace $extraer(\textit{f[1]})
